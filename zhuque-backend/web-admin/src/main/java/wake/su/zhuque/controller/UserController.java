...
public class UserController {
  ...
  public Result<Void> updateThemePreference(@PathVariable("userId") Long userId,
   ...
   */
  @Operation(summary = "重置用户密码", description = "重置用户密码，可选择自定义密码或使用默认规则（yyyyMMdd+手机号）。成功后返回明文密码，请妥善保管。")
  @PostMapping("/{userId}/reset-password")
  public Result<ResetPasswordResponse> resetPassword(
      @Parameter(description = "用户ID", required = true, example = "3") @PathVariable Long userId,
  ...
  public Result<Void> assignRoles(@PathVariable("userId") Long userId, @RequestBody Map<String, List<Long>> request) {
   ...
   * @return 是否成功
   */
  @Operation(summary = "修改密码", description = "用户自己修改密码，需要输入原密码验证")
