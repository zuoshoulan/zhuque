...
public class PermissionController {
   ...
   * 创建权限
   */
  @Operation(summary = "创建权限", description = "创建新的权限，支持路由、按钮、接口三种类型")
  ...
  public Result<Long> createPermission(
      @Parameter(description = "权限创建请求", required = true) @Valid @RequestBody PermissionCreateRequest request) {
   ...
   * 删除权限
   */
  @Operation(summary = "删除权限", description = "删除指定权限，如果存在子权限则无法删除")
   ...
   * 获取权限树
   */
  @Operation(summary = "获取权限树", description = "获取树形结构的权限列表，包含父子关系")
  @ApiResponse(responseCode = "200", description = "查询成功")
  @GetMapping("/tree")
   ...
   * 获取权限列表（平铺）
   */
  @Operation(summary = "获取权限列表", description = "获取平铺的权限列表，不包含层级关系")
  @ApiResponse(responseCode = "200", description = "查询成功")
  @GetMapping("/list")
