...
public class FrequencyCapServiceImpl implements FrequencyCapService {
    ...
    private int calculateTtl(Integer period) {
        ...
        return switch (period) {
            ...
            case PERIOD_DAY -> (int) Duration.between(now, now.plusDays(1).withHour(0).withMinute(0).withSecond(0)).getSeconds();
            case PERIOD_WEEK -> {
                int daysUntilMonday = 7 - now.getDayOfWeek().getValue();
                if (daysUntilMonday == 0) daysUntilMonday = 7;
                yield (int) Duration.between(now, now.plusDays(daysUntilMonday).withHour(0).withMinute(0).withSecond(0)).getSeconds();
            }
            case PERIOD_MONTH -> {
                int daysUntilMonthEnd = now.getMonth().length(now.toLocalDate().isLeapYear()) - now.getDayOfMonth() + 1;
                yield (int) Duration.between(now, now.plusDays(daysUntilMonthEnd).withHour(0).withMinute(0).withSecond(0)).getSeconds();
            }
            default -> 86400;  // 默认1天
        };
    }
