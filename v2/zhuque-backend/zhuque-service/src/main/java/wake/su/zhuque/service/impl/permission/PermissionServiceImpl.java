...
public class PermissionServiceImpl implements PermissionService {
  ...
  public List<PermissionVO> getPermissionTree() {
    List<SysPermissionDO> all = permissionMapper
        .selectList(new LambdaQueryWrapper<SysPermissionDO>().orderByAsc(SysPermissionDO::getSortOrder));
  ...
  public List<String> getUserPermissionCodes(Long userId) {
    ...
    List<Long> roleIds = userRoles.stream().map(SysUserRoleDO::getRoleId).collect(Collectors.toList());
    ...
    List<Long> permissionIds = rolePermissions.stream().map(SysRolePermissionDO::getPermissionId).distinct()
        .collect(Collectors.toList());

    // 查询权限详情
    List<SysPermissionDO> permissions = permissionMapper.selectList(new LambdaQueryWrapper<SysPermissionDO>()
        .in(SysPermissionDO::getId, permissionIds).eq(SysPermissionDO::getStatus, 1));

    // 转换为Set并缓存
    java.util.Set<String> permissionCodes = permissions.stream().map(SysPermissionDO::getPermissionCode)
  ...
  private List<PermissionVO> buildTree(List<PermissionVO> all, Long parentId) {
    return all.stream().filter(vo -> vo.getParentId().equals(parentId)).peek(vo -> {
