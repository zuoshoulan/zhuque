...
public class MenuServiceImpl implements MenuService {
  ...
  public List<MenuVO> getUserMenuTree(Long userId) {
    ...
    // TODO: 根据角色查询菜单权限
    // 简化实现：返回所有启用的菜单
    List<SysMenuDO> allMenus = menuMapper.selectList(new LambdaQueryWrapper<SysMenuDO>().eq(SysMenuDO::getStatus, 1)
        .eq(SysMenuDO::getVisible, 1).orderByAsc(SysMenuDO::getSortOrder));

    List<MenuVO> voList = allMenus.stream().map(this::convertToVO).collect(Collectors.toList());
  ...
  public List<MenuVO> getAllMenuTree() {
    List<SysMenuDO> allMenus = menuMapper.selectList(
        new LambdaQueryWrapper<SysMenuDO>().eq(SysMenuDO::getStatus, 1).orderByAsc(SysMenuDO::getSortOrder));
  ...
  public List<MenuVO> getAllMenuList() {
    ...
    return allMenus.stream().map(this::convertToVO).collect(Collectors.toList());
  }
