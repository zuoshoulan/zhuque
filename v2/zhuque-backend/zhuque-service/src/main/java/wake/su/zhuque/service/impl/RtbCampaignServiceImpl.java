...
public class RtbCampaignServiceImpl implements RtbCampaignService {
    ...
    public CampaignVO detail(Long id) {
        ...
        if (campaign.getLifetimeBudget().compareTo(BigDecimal.ZERO) > 0) {
            int percent = campaign.getLifetimeBudgetUsed()
                    .multiply(new BigDecimal("100"))
                    .divide(campaign.getLifetimeBudget(), 0, RoundingMode.HALF_UP)
                    .intValue();
    ...
    /**
     * 计算显示状态（包含已完成判断）
     * 优先级：预算耗尽 > 投放时间耗尽 > 待开始 > 进行中 > 暂停 > 草稿
     */
    private void setDisplayStatus(CampaignListVO vo, RtbCampaignDO campaign) {
        LocalDateTime now = LocalDateTime.now();
        boolean budgetExhausted = campaign.getLifetimeBudgetUsed()
                .compareTo(campaign.getLifetimeBudget()) >= 0;
