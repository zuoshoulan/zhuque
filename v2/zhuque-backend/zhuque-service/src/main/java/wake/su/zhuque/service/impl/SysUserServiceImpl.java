...
public class SysUserServiceImpl implements SysUserService {
  ...
  public SysUserDO getByAccount(String account) {
    ...
    // 第一版：account 仅支持手机号
    return sysUserMapper.selectOne(new LambdaQueryWrapper<SysUserDO>().eq(SysUserDO::getPhone, account));
  ...
  public ResetPasswordResponse resetPassword(Long userId, String newPassword) {
    ...
    // 构建响应（返回明文密码，仅此一次）
    return ResetPasswordResponse.builder().userId(user.getId()).username(user.getUsername()).phone(user.getPhone())
        .password(finalPassword) // 明文密码，仅此一次返回
        .passwordType(passwordType).build();
  }
  ...
  public List<RoleVO> getUserRoles(Long userId) {
    ...
    // 转换为VO
    return roles.stream().filter(role -> role.getStatus() == 1) // 只返回启用的角色
        .map(role -> {
  ...
  public boolean assignRoles(Long userId, List<Long> roleIds) {
    ...
    if(roleIds != null && !roleIds.isEmpty()) {
      ...
      LocalDateTime now = LocalDateTime.now();
      List<SysUserRoleDO> userRoleList = roleIds.stream().map(roleId -> {
