...
public class CacheController {
  ...
  public Result<Map<String, Object>> refreshUserPermissionsBatch(@RequestBody Map<String, List<Long>> request) {
    ...
    return Result.success(Map.of("count", userIdSet.size(), "message", "成功清除 " + userIdSet.size() + " 个用户的权限缓存"));
  }

  /**
   * 清除所有权限缓存 警告：此操作会影响所有用户的权限验证，仅在必要时使用
   *
   * @return 是否成功
   */
  @Operation(summary = "清除所有权限缓存", description = "清除所有用户的权限缓存。警告：此操作会影响所有用户的权限验证，仅在必要时使用！")
  @DeleteMapping("/all")
  ...
  public Result<Map<String, Object>> checkUserPermissionCache(
    ...
    return Result.success(Map.of("userId", userId, "hasCache", exists, "message", exists ? "权限缓存存在" : "权限缓存不存在"));
  }
