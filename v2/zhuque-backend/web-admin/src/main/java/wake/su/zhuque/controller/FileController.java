...
/**
 * 文件管理Controller 用于文件的存储在数据库中的上传和下载 支持 HTTP 缓存，适合竞价场景
 */
...
public class FileController {
  ...
  public Result<FileUploadResponse> upload(@RequestParam("file") MultipartFile file) {
    ...
    // 构建响应 - 返回 id（管理后台用）
    FileUploadResponse response = FileUploadResponse.builder().fileId(fileRecord.getId())
        .fileName(fileRecord.getFileName()).fileSize(fileRecord.getFileSize()).fileType(fileRecord.getFileType())
        .width(fileRecord.getWidth()).height(fileRecord.getHeight()).build();
  ...
  public ResponseEntity<byte[]> getFile(@PathVariable String fileUuid) {
    // 根据fileUuid查询文件
    RtbFileDO fileRecord = fileMapper
