...
public class AuthController {
    ...
    @Operation(
        summary = "用户登录",
        description = "使用账号（手机号）和密码登录，成功后返回JWT Token。第一版仅支持手机号登录。"
    )
    @ApiResponse(responseCode = "200", description = "登录成功")
    @ApiResponse(responseCode = "401", description = "用户名或密码错误")
    @PostMapping("/login")
    public OldResult<LoginResponse> login(
    ...
    @Operation(
        summary = "用户登出",
        description = "用户退出登录，Token失效"
    )
    @ApiResponse(responseCode = "200", description = "登出成功")
    @PostMapping("/logout")
    public OldResult<Void> logout(
            @Parameter(description = "JWT Token", example = "Bearer eyJhbGciOiJIUzI1NiJ9...")
