package wake.su.zhuque.common.security;
 ...
 *
 * <p><strong>超级管理员特性：</strong></p>
 * <ul>
 *   <li>禁止删除（用户、角色、权限数据受保护）</li>
 *   <li>自动拥有所有权限</li>
 *   <li>绕过所有权限检查</li>
 * </ul>
 *
...
public class SuperAdminHolder {
     ...
     * @param userId 用户ID
     * @param config 超级管理员配置
     * @param requiredPermission 所需权限（不使用，超级管理员自动拥有所有权限）
     * @return true 如果是超级管理员
     */
     ...
     * @param logical true=AND（需要所有权限）, false=OR（满足任意一个即可）
     * @return true 如果是超级管理员
     */
     ...
     * @param config 超级管理员配置
     * @throws BusinessException 如果列表中包含超级管理员
     */
